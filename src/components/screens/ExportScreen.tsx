...
export const ExportScreen: React.FC = () => {
  ...
  const handleDownloadCSV = () => {
    ...
    showToast(`Downloaded ${filename} (${exportablePurchases.length} rows)`);
  };
  ...
  const handleSaveCsvToDrive = async () => {
    if (exportablePurchases.length === 0) {
      showToast('No entries to export to Google Drive');
      return;
    }
    const today = new Date().toISOString().split('T')[0];
    const filename = `spoton_spending_${today}.csv`;
  ...
  const handlePromptRestoreFromDrive = (file: DriveFileItem) => {
    setConfirmModal({
      ...
      description: `Restoring will update your app's purchase records, presets, and budget preferences with data from "${file.name}".`,
      details: [
        `File Name: ${file.name}`,
        `Saved: ${new Date(file.modifiedTime).toLocaleString()}`,
        `Current Records: ${purchases.length} items will be updated`,
      ],
  ...
  return (
    <div className="space-y-5 pb-28">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-slate-900 dark:text-white">Export & Cloud Sync</h1>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Sync with Google Drive and export reports in standard formats
        </p>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Securely store and restore backups directly on your Google Drive
              </p>
            </div>
          </div>
        ...
        {!isGoogleConnected ? (
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-850 border border-slate-100 dark:border-slate-800 space-y-3">
            <div className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              Connect your Google Account to automatically backup your purchases, sync data between your devices, and export spreadsheets directly to Google Drive.
            </div>
            <GoogleSignInButton
          ...
          <div className="space-y-3.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              ...
              >
                ...
                <span>1-Tap Drive Backup ({purchases.length} Items)</span>
              </button>
              ...
              >
                ...
                <span>Save CSV to Google Drive</span>
              </button>
            </div>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
              ...
              {isLoadingDriveFiles ? (
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-850 text-center text-xs text-slate-500">
                  No SpotOn backups found in Google Drive yet. Tap "1-Tap Drive Backup" above to create one!
                </div>
              ) : (
      ...
      <div className="bg-slate-50 dark:bg-slate-900/60 rounded-3xl p-5 border border-slate-100 dark:border-slate-800 space-y-4">
        <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">
          ...
          <span>Local Export & Filters</span>
        </div>
        ...
        <div className="flex items-center justify-between p-3 rounded-xl bg-white dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700 text-xs">
          <span className="text-slate-500 dark:text-slate-400">
            Selected: <strong className="text-slate-800 dark:text-slate-200">{exportablePurchases.length}</strong> items
          </span>
          <span className="text-slate-500 dark:text-slate-400">
            Total value: <strong className="text-slate-900 dark:text-white font-bold">{formatCurrency(totalExportSum, settings.currencySymbol)}</strong>
          </span>
        </div>
