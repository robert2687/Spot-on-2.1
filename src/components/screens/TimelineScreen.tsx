...
export const TimelineScreen: React.FC = () => {
  const { purchases, settings, openAddModal, deletePurchase } = useSpotOn();
  ...
  return (
    <div className="space-y-4 pb-28">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">Timeline</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {filteredPurchases.length} logged purchase{filteredPurchases.length === 1 ? '' : 's'}
          </p>
        </div>
      ...
      {groupedPurchases.length === 0 ? (
        <div className="bg-slate-50 dark:bg-slate-900/60 rounded-2xl p-8 border border-slate-100 dark:border-slate-800 text-center">
          <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">No purchases found</p>
          ...
          {groupedPurchases.map((group) => (
            <div key={group.dateKey} className="space-y-1.5">
              ...
              <div className="flex items-center justify-between px-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
                ...
                <span>Day total: {formatCurrency(group.dayTotal, settings.currencySymbol)}</span>
              </div>
